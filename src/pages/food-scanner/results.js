  ...
  return `
    <div class="card ${isGood ? 'combo-good' : 'combo-bad'} mb-3">
      <div class="flex items-center gap-2 mb-2">
        ...
        <h3 class="text-sm">${esc(combo.title)}</h3>
        <span class="badge ${isGood ? 'badge-green' : 'badge-coral'}" style="margin-left:auto;">${isGood ? 'Optimal' : 'Avoid'}</span>
      </div>
      <p class="text-xs text-secondary" style="line-height:1.5;">${esc(combo.explanation)}</p>
    </div>
  `;
}
...
function renderCorrectionSection(foods, food) {
  ...
  return `
    <div class="card">
      <h3 class="h4 mb-2">Something wrong?</h3>
      ...
      <div class="flex-col gap-2">
        ${items.map((item, i) => `
          ...
          <div id="correct-input-${i}" style="display:none;padding:var(--space-2);background:var(--surface-2);border-radius:var(--radius-md);">
            <label for="correct-text-${i}" class="block text-xs text-secondary mb-2">What is this food actually?</label>
            <div class="flex gap-2">
              <input type="text" id="correct-text-${i}" placeholder="e.g. salmon fillet" class="flex-1 text-sm p-2 bg-surface-3 border rounded-md text-primary" />
    ...
    <div class="stagger-children flex-col gap-4">
      <div class="card">
        <div class="flex-between mb-3">
          <div>
            <h2 class="text-lg mb-1">${esc(food.name)}</h2>
          ...
          <div class="text-center">
            <div id="total-calories-display" class="font-heading text-3xl" style="font-weight:var(--weight-extrabold);color:var(--accent-teal);">${Math.round(food.calories || 0)}</div>
            ...
            <div class="portion-item" data-index="${i}" data-is-drink="${drink ? '1' : '0'}"
              ...
              <div class="flex-between mb-2">
                <div>
                  ...
                  <div class="text-xs text-tertiary flex items-center gap-2">
                    ${item.confidence ? `${(Number(item.confidence) * 100).toFixed(0)}% confidence` : ''}
                    ${item.confidence >= 0.85 ? '' :
                      item.confidence >= 0.60 ? `<span class="text-primary text-xs font-semibold" style="background:var(--viz-amber);padding:1px 6px;border-radius:4px;">Uncertain</span>` :
                        `<span class="text-xs font-semibold" style="background:var(--error);color:var(--text-inverse);padding:1px 6px;border-radius:4px;">Low confidence</span>`}
                  </div>
                </div>
                <div class="text-right">
                  <span id="grams-display-${i}" class="font-heading text-lg font-bold" style="color:var(--accent-teal);">${displayVal}</span>
                  <div id="kcal-display-${i}" class="text-tertiary text-xs">${Math.round(item.nutrients?.calories || 0)} kcal</div>
                </div>
              </div>
              <input type="range" id="portion-slider-${i}" min="${sliderMin}" max="${sliderMax}" step="${sliderStep}" value="${Number(baseVal) || 150}" aria-label="Portion size for ${esc(item.name)}" class="w-full cursor-pointer" style="accent-color:var(--accent-teal);" />
...
<div class="item-actions" style="margin-top:var(--space-2);padding:var(--space-2);background:${item.confidence < 0.60 ? 'var(--error-dim)' : 'var(--surface-2)'};border-radius:var(--radius-md);">
  ...
  <div class="flex gap-2 flex-wrap">
    <button type="button" class="item-action-confirm flex-1 text-xs rounded-md text-green cursor-pointer font-semibold" data-index="${i}" style="padding:4px 8px;border:1px solid var(--viz-green);background:transparent;" aria-label="Confirm ${esc(item.name)} is correct">Correct</button>
    <button type="button" class="item-action-replace flex-1 text-xs rounded-md text-amber cursor-pointer font-semibold" data-index="${i}" style="padding:4px 8px;border:1px solid var(--viz-amber);background:transparent;" aria-label="Replace ${esc(item.name)}">Replace</button>
    <button type="button" class="item-action-remove flex-1 text-xs rounded-md text-error cursor-pointer font-semibold" data-index="${i}" style="padding:4px 8px;border:1px solid var(--error);background:transparent;" aria-label="Remove ${esc(item.name)} from this meal">Remove</button>
  </div>
  <div class="item-replace-input" data-index="${i}" style="display:none;margin-top:var(--space-2);">
    <div class="flex gap-2">
      <input type="text" class="item-replace-text flex-1 text-xs p-2 bg-surface-3 border rounded-md text-primary" data-index="${i}" placeholder="What is this food?" aria-label="Replacement food name">
      <button type="button" class="item-replace-save text-xs rounded-md text-primary cursor-pointer font-semibold" data-index="${i}" style="padding:4px 10px;background:var(--accent-teal);border:none;">Go</button>
    </div>
  </div>
  <input type="checkbox" id="include-item-${i}" checked aria-hidden="true" tabindex="-1" style="display:none;">
        ...
        <div class="mt-3" style="border-top:1px solid var(--border);padding-top:var(--space-3);">
  <div id="add-food-search" style="display:none;margin-bottom:var(--space-2);">
    <div class="flex gap-2">
      ...
      <input type="text" id="add-food-input" placeholder="Search for a food to add..." class="flex-1 text-sm bg-surface-2 border rounded-md text-primary" style="padding:var(--space-2) var(--space-3);">
      <button type="button" id="add-food-search-btn" class="rounded-md text-primary cursor-pointer text-xs font-semibold" style="padding:var(--space-2) var(--space-3);background:var(--accent-teal);border:none;">Search</button>
    </div>
    <div id="add-food-results" aria-live="polite" class="flex-col gap-2 mt-2"></div>
  </div>
  <button type="button" id="add-missing-food-btn" aria-expanded="false" aria-controls="add-food-search" class="w-full p-2 rounded-md text-secondary text-xs cursor-pointer flex-center gap-2" style="border:1px dashed var(--border);background:transparent;">
    <span style="font-size:16px;" aria-hidden="true">+</span> Add missing food
      ...
      <div class="card">
        <h3 class="h4 mb-4">Macronutrients</h3>
        <div class="flex items-center gap-5">
          ...
          { percent: fatPct, color: 'var(--accent-coral)' },
            ...
            <div class="chart-center-label">
              <div class="value text-md" id="total-macro-display">${totalMacro}g</div>
      ...
      <div class="card">
        <h3 class="h4 mb-3">Micronutrients</h3>
        ${food.micronutrients.map(m => `
          <div class="flex-between" style="padding:var(--space-2) 0;">
            ...
            <div class="flex items-center gap-3">
              ...
              <span class="text-xs text-tertiary text-right" style="min-width:32px;">${Number(m.rda) || 0}%</span>
            </div>
          </div>
        `).join('')}
      ...
      <div class="card">
        <div class="flex-between">
          <div>
            <h3 class="h4 mb-1">Digestibility Score</h3>
      ...
      <div class="section-heading"><h2 class="h3">Food Combination Analysis</h2></div>
      ${(combinations || []).map(c => renderComboCard(c)).join('')}

      <div class="section-heading"><h2 class="h3">Eastern Medicinal Analysis</h2></div>
      ...
      <p class="text-xs text-tertiary text-center" style="margin-top:calc(-1 * var(--space-2));">

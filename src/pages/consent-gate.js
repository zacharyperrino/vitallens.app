...
export function renderConsentGate() {
    ...
    const links = CONSENT_DOCS
        .map(d => `<a href="${d.href}" target="_blank" style="color:var(--accent);">${d.label}</a>`)
        .join(', ');

    content.innerHTML = `
    <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:var(--space-6);">
      <div class="card" style="width:100%;max-width:440px;">
        <h2 style="margin-bottom:var(--space-3);">Before you continue</h2>
        ...
        <label style="display:flex;gap:var(--space-2);align-items:flex-start;font-size:var(--text-sm);cursor:pointer;margin-bottom:var(--space-2);">
        ...
        <div id="consent-error" style="display:none;color:var(--viz-amber);font-size:var(--text-xs);margin-bottom:var(--space-2);"></div>
        <button id="consent-continue" class="btn btn-glass btn-block" style="margin-top:var(--space-3);">Agree & Continue</button>
      </div>
    </div>`;

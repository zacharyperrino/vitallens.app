...
export async function fetchAndRenderDelta(scanType, currentScore) {
  ...
  try {
    ...
    if (!previous || !Number.isFinite(Number(previous.score))) {
      deltaCard.innerHTML = `
        <div class="card card-sm" style="border-left:3px solid var(--viz-green);background:var(--viz-green-dim);">
          <div class="text-xs text-secondary font-semibold">First ${esc(modeLabel.toLowerCase())} check-in saved — next time you'll see how it compares.</div>
        </div>`;
      return;
    }
    ...
    deltaCard.innerHTML = `
      <div class="card card-sm" style="border-left:3px solid ${tone.color};">
        <div class="flex-between">
          <div>
            <div class="text-xs text-tertiary" style="margin-bottom:2px;">vs your previous ${esc(modeLabel.toLowerCase())} check-in · ${esc(prevDate)}</div>
          ...
          <div class="flex gap-3 items-center">
            <div class="text-center">
              ...
              <div class="font-heading text-lg text-secondary" style="font-weight:700;">${prevScore}</div>
            </div>
    ...
    deltaCard.innerHTML = `
      <div class="empty-state card card-sm p-3" role="alert">
        <h2 class="text-sm">Couldn't compare with your previous check-in</h2>
...
function renderScoreHeader(m, score, caption, badges = '') {
  ...
  return `
    <div class="card text-center">
      <h2 class="h3 mb-2">${esc(m.label)} Check-In</h2>
...
function statTile(label, value, color) {
  return `
    <div class="p-2 bg-surface-2 rounded-md">
      <div class="text-xs text-tertiary" style="margin-bottom:2px;">${esc(label)}</div>
...
export function renderFaceResults(r, m) {
  ...
  return `
    ...
    ${r.fitzpatrick_type ? `
    <div class="card">
      <h3 class="h4 mb-3">Skin Profile</h3>
      ...
      ${r.fitzpatrick_notes ? `<p class="text-xs text-secondary mb-2" style="line-height:1.5;">${esc(r.fitzpatrick_notes)}</p>` : ''}
    </div>` : ''}

    <!-- Structure notes -->
    ${(r.forehead_lines || r.nasolabial_folds || r.jowling || r.temporal_hollowing) ? `
    <div class="card">
      <h3 class="h4 mb-3">Structure Notes</h3>
      <div class="flex-col gap-2">
        ...
        <div style="padding:var(--space-2) 0;border-bottom:1px solid var(--border);">
          <div class="flex justify-between" style="margin-bottom:4px;">
            ...
            <span class="text-xs font-semibold">Horizontal: ${words(r.forehead_lines.horizontal)} · Vertical: ${words(r.forehead_lines.vertical_glabellar)}</span>
          </div>
          ${r.forehead_lines.wellness_note ? `<p class="text-tertiary text-xs">${esc(r.forehead_lines.wellness_note)}</p>` : ''}
        </div>` : ''}
        ${r.nasolabial_folds ? `
        <div class="flex justify-between" style="padding:var(--space-2) 0;border-bottom:1px solid var(--border);">
          <span class="text-secondary text-xs">Smile Lines</span>
          <span class="text-xs font-semibold">${words(r.nasolabial_folds)}</span>
        </div>` : ''}
        ${r.jowling ? `
        <div class="flex justify-between" style="padding:var(--space-2) 0;border-bottom:1px solid var(--border);">
          <span class="text-secondary text-xs">Jawline Softness</span>
          <span class="text-xs font-semibold">${words(r.jowling)}</span>
        </div>` : ''}
        ...
        <div class="flex justify-between" style="padding:var(--space-2) 0;">
          ...
          <span class="text-xs font-semibold">${words(r.temporal_hollowing)}</span>
        </div>` : ''}
      </div>
    </div>` : ''}
    ...
    <div class="section-heading"><h2 class="h3">Facial Zones</h2></div>
    ${zoneOrder.filter(z => zones[z] && typeof zones[z] === 'object').map(z => {
      ...
      if (z === 'periorbital') {
        ...
        <div class="card card-sm" style="border-left:3px solid ${toneFor(zone.dark_circles || 'clear').color};">
          ...
          ${zone.wellness_signal ? `<p class="text-xs text-green">${esc(zone.wellness_signal)}</p>` : ''}
        </div>`;
      }
      return `
      <div class="card card-sm" style="border-left:3px solid ${tone.color};">
        ...
        <p class="text-xs text-secondary mb-1" style="line-height:1.5;">${esc(zone.condition || '')}</p>
        ${zone.wellness_signal ? `<p class="text-xs text-green">${esc(zone.wellness_signal)}</p>` : ''}
        ${zone.possible_system ? `<p class="text-xs text-green">${esc(zone.possible_system)}</p>` : ''}
      </div>`;
    }).join('')}` : ''}
    ...
    <div class="card">
      <h3 class="h4 mb-3">Skin Tone Notes</h3>
      <div class="flex flex-wrap gap-2">
        ${Object.entries(r.discoloration).filter(([, v]) => v === true).map(([k]) =>
          `<span class="text-xs rounded-full text-amber" style="padding:4px 10px;background:var(--viz-amber-dim);">${words(k)}</span>`
        ).join('')}
        ${r.discoloration.hyperpigmentation_pattern && r.discoloration.hyperpigmentation_pattern !== 'none' ? `<span class="text-xs rounded-full text-amber" style="padding:4px 10px;background:var(--viz-amber-dim);">${words(r.discoloration.hyperpigmentation_pattern)}</span>` : ''}
      </div>
    </div>` : ''}
    ...
    <div class="card">
      <h3 class="h4 mb-3">Other Notes</h3>
      <div class="flex-col gap-2">
        ${r.eyebrow_notes && r.eyebrow_notes !== 'normal' ? `<div class="flex justify-between"><span class="text-secondary text-xs">Eyebrows</span><span class="text-xs font-semibold text-amber">${words(r.eyebrow_notes)}</span></div>` : ''}
        ${r.lip_notes && r.lip_notes !== 'normal' ? `<div class="flex justify-between"><span class="text-secondary text-xs">Lips</span><span class="text-xs font-semibold text-amber">${words(r.lip_notes)}</span></div>` : ''}
        ${r.facial_puffiness && r.facial_puffiness !== 'none' ? `<div class="flex justify-between"><span class="text-secondary text-xs">Facial Puffiness</span><span class="text-xs font-semibold text-amber">${words(r.facial_puffiness)}</span></div>` : ''}
        ${r.facial_symmetry && r.facial_symmetry !== 'normal' ? `<div class="flex justify-between"><span class="text-secondary text-xs">Symmetry</span><span class="text-xs font-semibold">${words(r.facial_symmetry)}</span></div>` : ''}
      </div>
    </div>` : ''}
...
export function renderTongueResults(r, m) {
  ...
  return `
    ...
    ${body ? `
    <div class="card">
      <h3 class="h4 mb-3">Tongue Body</h3>
      <div class="grid gap-2 mb-3" style="grid-template-columns:1fr 1fr;">
      ...
      ${body.cracks?.present ? `
      <div class="mt-2 p-2 rounded-md" style="background:var(--viz-amber-dim);">
        <p class="text-xs text-amber font-semibold">Cracks noted${Array.isArray(body.cracks.locations) && body.cracks.locations.length ? `: ${esc(body.cracks.locations.join(', '))}` : ''}</p>
        ${body.cracks.significance ? `<p class="text-xs text-secondary" style="margin-top:4px;">${esc(body.cracks.significance)}</p>` : ''}
      </div>` : ''}
    </div>` : ''}

    ${coating ? `
    <div class="card">
      <h3 class="h4 mb-3">Tongue Coating</h3>
    ...
    <div class="card" style="border-left:3px solid var(--viz-green);">
      <h3 class="h4 mb-2">Traditional (TCM) Pattern</h3>
    ...
    `<span class="text-xs text-green" style="padding:2px 8px;border-radius:4px;background:var(--viz-green-dim);">${esc(o)}</span>`
  ).join('')}
      </div>` : ''}
    ...
    <div class="card card-sm" style="border-left:3px solid var(--viz-amber);">
      <h3 class="h4 mb-2">Nutrition — something to explore</h3>
      ${r.nutritional_deficiency_flags.map(d =>
    `<div class="text-xs text-secondary" style="padding:var(--space-1) 0;">• ${esc(typeof d === 'string' ? d : d?.text || d?.nutrient || '')}</div>`
  ).join('')}
    </div>` : ''}
  ...
  return `
    ...
    <div class="card">
      ...
      <div class="flex-col gap-1">
        ...
        <div class="flex justify-between" style="padding:var(--space-1) 0;border-bottom:1px solid var(--border);">
          <span class="text-secondary text-xs">${esc(item.label)}</span>
          <span class="text-xs font-semibold">${words(item.value)}</span>
        </div>`).join('')}
      </div>
      ${posture.primary_observation ? `<p class="text-xs text-amber mt-3">${esc(posture.primary_observation)}</p>` : ''}
    </div>` : ''}
    ...
    <div class="card card-sm" style="border-left:3px solid var(--viz-amber);">
      <div class="flex-between mb-2">
        ...
        ${apt.severity ? `<span class="text-xs text-amber" style="padding:1px 6px;border-radius:4px;background:var(--viz-amber-dim);">${words(apt.severity)}</span>` : ''}
      </div>
      ${apt.functional_note ? `<p class="text-secondary text-xs">${esc(apt.functional_note)}</p>` : ''}
      ${Array.isArray(apt.indicators_observed) && apt.indicators_observed.length > 0 ? `<p class="text-xs text-tertiary" style="margin-top:4px;">${esc(apt.indicators_observed.join(' · '))}</p>` : ''}
    </div>` : ''}

    ${muscle ? `
    <div class="card">
      <h3 class="h4 mb-3">Muscle Balance Notes</h3>
      <div class="flex-col gap-2">
        ${[
    ...
    <div class="card card-sm">
      ...
      ${breathing.wellness_note ? `<p class="text-xs text-secondary mt-1">${esc(breathing.wellness_note)}</p>` : ''}
    </div>` : ''}

    ${comp ? `
    <div class="card">
      <h3 class="h4 mb-3">Build Notes</h3>
      <div class="grid gap-2" style="grid-template-columns:1fr 1fr;">
    ...
    <div class="card card-sm">
      <h3 class="h4 mb-2">Puffiness Notes</h3>
      <div class="flex-col gap-1">
        ${[
        ...
        <div class="flex justify-between">
          <span class="text-secondary text-xs">${esc(item.label)}</span>
          <span class="text-xs font-semibold text-amber">${words(item.value)}</span>
        </div>`).join('')}
      </div>
    ...
    <div class="section-heading"><h2 class="h3">Wellness Observations</h2></div>
    ${r.wellness_observations.map(obs => `
      <div class="card card-sm" style="border-left:3px solid var(--text-tertiary);">
        ...
        <p class="text-xs text-secondary" style="line-height:1.5;">${esc(obs?.significance || '')}</p>
      </div>`).join('')}` : ''}
...
function renderSignals(flags, heading) {
  ...
  return `
    <div class="section-heading"><h2 class="h3">${esc(heading)}</h2></div>
    ${flags.map(flag => {
    ...
    return `
      <div class="card card-sm" style="border-left:3px solid var(--text-tertiary);">
        ...
        ${detail ? `<p class="text-xs text-secondary" style="line-height:1.5;">${esc(detail)}</p>` : ''}
        ${confidence ? `<p class="text-xs text-tertiary mt-1">Confidence: ${esc(confidence)}</p>` : ''}
      </div>`;
  }).join('')}`;
}
...
function renderRecommendations(recs) {
  ...
  return `
    <div class="section-heading"><h2 class="h3">Worth a Look</h2></div>
    ${recs.map(rec => {

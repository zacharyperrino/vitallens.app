...
function renderTCMPatternCard() {
  if (!tcmProfile) return `<div class="card"><h3>Your TCM Pattern</h3><p class="text-secondary text-sm">Analyze meals to build your accumulated constitution pattern.</p></div>`;
...
function renderDoshaProfile(doshaKey) {
  ...
  return `<div class="stagger-children flex-col gap-4">
    ...
    <div class="card"><h4 class="mb-2">Body Type (traditional description)</h4><p class="text-sm">${esc(d.bodyType)}</p></div>
    <div class="card"><h4 class="mb-2">Personality (traditional description)</h4><p class="text-sm">${esc(d.personality)}</p></div>
    <div class="card"><h4 class="mb-2">Signs traditionally linked to imbalance</h4>
    ...
    <div class="card"><h4 class="mb-2">Lifestyle practices traditionally suggested</h4>
...
function renderFaceMap() {
  return `<div class="stagger-children flex-col gap-4">
    <div class="card text-center">
      <h4 class="mb-2">Chinese Face Mapping</h4>
  ...
  document.querySelectorAll('.face-map-zone, .zone-list-item').forEach(el => {
    el.addEventListener('click', () => {
      ...
      detail.innerHTML = `<div class="card animate-fade-in-up" style="border-left:3px solid var(--viz-green);">
            <h4 class="mb-1">${esc(zone.name)}</h4>

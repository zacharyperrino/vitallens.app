...
- **Backend**: single Express process (`server/server.js`) — express, helmet, cors, morgan, express-rate-limit, multer (uploads), zod (AI response validation), stripe, web-push, `@sentry/node`, `@upstash/redis` (context cache), pg, dotenv. Started as `node --import ./instrument.js server.js` (`npm start`, `Procfile`, `railway.json`) so Sentry hooks Express before it loads; multer is 2.x.
